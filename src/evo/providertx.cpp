...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (!ptx.collateralOutpoint.hash.IsNull()) {
        Coin coin;
        if (!GetUTXOCoin(ptx.collateralOutpoint, coin) || coin.out.nValue != 314000 * COIN) {
        ...
        if (!CThoughtAddress(collateralTxDest).GetKeyID(keyForPayloadSig)) {
        ...
        if (tx.vout[ptx.collateralOutpoint.n].nValue != 314000 * COIN) {
            return state.DoS(10, false, REJECT_INVALID, "bad-protx-collateral");
        }
...
std::string CProRegTx::MakeSignString() const
{
    ...
    s += CThoughtAddress(keyIDOwner).ToString() + "|";
    s += CThoughtAddress(keyIDVoting).ToString() + "|";
...
std::string CProRegTx::ToString() const
{
    ...
    if (ExtractDestination(scriptPayout, dest)) {
        payee = CThoughtAddress(dest).ToString();
    }
    ...
    return strprintf("CProRegTx(nVersion=%d, collateralOutpoint=%s, addr=%s, nOperatorReward=%f, ownerAddress=%s, pubKeyOperator=%s, votingAddress=%s, scriptPayout=%s)",
        nVersion, collateralOutpoint.ToStringShort(), addr.ToString(), (double)nOperatorReward / 100, CThoughtAddress(keyIDOwner).ToString(), pubKeyOperator.ToString(), CThoughtAddress(keyIDVoting).ToString(), payee);
}
...
void CProRegTx::ToJson(UniValue& obj) const
{
    ...
    obj.push_back(Pair("ownerAddress", CThoughtAddress(keyIDOwner).ToString()));
    obj.push_back(Pair("votingAddress", CThoughtAddress(keyIDVoting).ToString()));

    CTxDestination dest;
    if (ExtractDestination(scriptPayout, dest)) {
        CThoughtAddress thoughtAddress(dest);
        obj.push_back(Pair("payoutAddress", thoughtAddress.ToString()));
    }
...
void CProUpServTx::ToJson(UniValue& obj) const
{
    ...
    if (ExtractDestination(scriptOperatorPayout, dest)) {
        CThoughtAddress thoughtAddress(dest);
        obj.push_back(Pair("operatorPayoutAddress", thoughtAddress.ToString()));
    }
    obj.push_back(Pair("inputsHash", inputsHash.ToString()));
}
...
std::string CProUpRegTx::ToString() const
{
    ...
    return strprintf("CProUpRegTx(nVersion=%d, proTxHash=%s, pubKeyOperator=%s, votingAddress=%s, payoutAddress=%s)",
        nVersion, proTxHash.ToString(), pubKeyOperator.ToString(), CThoughtAddress(keyIDVoting).ToString(), payee);
}
