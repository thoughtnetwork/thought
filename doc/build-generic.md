GENERIC BUILD NOTES
====================
Some notes on how to build Thought Core based on the [depends](../depends/README.md) build system.

Note on old build instructions
------------------------------
In the past, the build documentation contained instructions on how to build Thought with system-wide installed dependencies
...
------------------------------------
Building the dependencies and Thought Core requires some essential build tools to be installed before. Please see
[build-unix](build-unix.md), [build-osx](build-osx.md) and [build-windows](build-windows.md) for details.

Building dependencies
---------------------
Thought inherited the `depends` folder from Thought, which contains all dependencies required to build Thought. These
dependencies must be built before Thought can actually be built. To do so, perform the following:
...
level. `./configure` of Thought Core will autodetect the presence of ccache and enable use of it. To disable ccache, use
